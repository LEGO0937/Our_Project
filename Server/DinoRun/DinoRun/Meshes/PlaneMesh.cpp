...
PlaneMesh::PlaneMesh() : CMesh()
{
}
PlaneMesh::PlaneMesh(float fx, float fy, float fz, float minX, float maxX, float minY, float maxY) : CMesh()
{
...
UnderPlaneMesh::UnderPlaneMesh() : CMesh()
{
}
UnderPlaneMesh::UnderPlaneMesh(float fx, float fy, float fz, float minX, float maxX, float minY, float maxY) : CMesh()
{
	m_nType = VERTEXT_POSITION | VERTEXT_TEXTURE_COORD0;
	m_nVertices = 6;
	m_pxmf3Positions = new XMFLOAT3[m_nVertices];
	...
	m_pxmf3Positions[i++] = XMFLOAT3(-fx, 0, fz);
	m_pxmf3Positions[i++] = XMFLOAT3(+fx, 0, fz);
	m_pxmf3Positions[i++] = XMFLOAT3(+fx, -fy*2, fz);
	m_pxmf3Positions[i++] = XMFLOAT3(+fx, -fy*2, fz);
	m_pxmf3Positions[i++] = XMFLOAT3(-fx, -fy*2, fz);
	m_pxmf3Positions[i++] = XMFLOAT3(-fx, 0, fz);
