...
CCamera::CCamera()
{
	...
	m_xmf4FogColor = XMFLOAT4{ 0.82f, 0.82f, 0.82f, 1.0f };
	m_fFogRange = 300.0f;
	m_fFogStart = 0.0f;
}
...
void CCamera::UpdateShaderVariables(ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	m_pcbMappedCamera->m_xmf4FogColor = m_xmf4FogColor;
	m_pcbMappedCamera->m_fFogRange = m_fFogRange;
	m_pcbMappedCamera->m_fFogStart = m_fFogStart;
...
void CThirdPersonCamera::Update(XMFLOAT3& xmf3LookAt, float fTimeElapsed)
{
	...
	if (m_pPlayer)
	{
		...
		float cXZ = 1.0f, kXZ = 17.5f;  //xz축
		{
			...
			if (length > 900)   //부스터시 캐릭터가 빨리이동하는것처럼 느끼게하기 위해 카메라와 플레이어의 거리를 늘린다.
			...
			//Result변수들은 이동 및 회전 후의 기존의 위치를 축으로하여 어느방향으로 이동하고 얼만큼 갔는지를 나타내는 변수.
			//기존위치를 기준으로한 상대적인 변화량임.
			...
			//위 부분에서 조화진동의 운동방정식을 적용하여 가해지는 힘을 구한다.
		...
		//---------------------------------
		/*
		탄성 반동이 안주어지는 경우의 코드 나중에 사용할 수 있으니 남김.
