	...
	nWidth, int nLength, int nBlockWidth, int nBlockLength, XMFLOAT3 xmf3Scale) : CGameObject(1)
{
	...
	pTerrainBaseTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Resources/Images/First_Map_Texture.dds", 0);

	CTexture *pTerrainDetailTexture = new CTexture(1, RESOURCE_TEXTURE2D, 0);
	pTerrainDetailTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Resources/Images/Detail_Texture.dds", 0);
	...
	pShader->CreateCbvSrvDescriptorHeaps(pd3dDevice, pd3dCommandList, 0, 2);
	pShader->CreateShaderResourceViews(pd3dDevice, pTerrainBaseTexture, 8, true);
	pShader->CreateShaderResourceViews(pd3dDevice, pTerrainDetailTexture, 9, true);

	material = new CMaterial(2);
	material->m_xmf4AmbientColor = XMFLOAT4(0.3, 0.3, 0.3, 1.0);
	material->m_xmf4DiffuseColor = XMFLOAT4(0.5, 0.7, 0.5, 1.0);
	
	material->SetShader(pShader);
	material->SetTexture(pTerrainBaseTexture, 0);
	material->SetTexture(pTerrainDetailTexture, 1);
	...
	for (int z = 0, zStart = 0; z < czBlocks; z++)
	{
		for (int x = 0, xStart = 0; x < cxBlocks; x++)
		{
			...
			CHeightMapGridMesh *pMeshInfo = new CHeightMapGridMesh(pd3dDevice, pd3dCommandList, xStart,
				zStart, nBlockWidth, nBlockLength, xmf3Scale, m_pHeightMapImage);

			pMeshInfo->CreateShaderVariables(pd3dDevice, pd3dCommandList);
