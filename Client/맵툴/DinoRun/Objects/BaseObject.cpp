...
{
	m_ppAnimationSets[nAnimationSet]->m_pCallbackFuncKeys[nKeyIndex].m_fTime = fKeyTime;
	...
	for (; ; )
	{
		...
		if (!strcmp(pstrToken, "<Material>:"))
		{
			...
			if (nTexture > 0)
			{
				...
				shader->CreateCbvSrvDescriptorHeaps(pd3dDevice, pd3dCommandList, 0, nTexture);
				//ReadStringFromFile(pInFile, pstrToken);
				for (int j = 0; j < nTexture; ++j)
				{
					ReadStringFromFile(pInFile, pstrToken);
					CTexture *pTexture = new CTexture(1, RESOURCE_TEXTURE2D, 0);
					pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, ConvertCHARtoWCHAR(pstrToken), 0);
					shader->CreateShaderResourceViews(pd3dDevice, pTexture, 8+j, true);
			...
			//	{
			//		pMaterial->m_ppTextures[i]->LoadTextureFromFile(pd3dDevice, pd3dCommandList, pstrToken, i, true);
			//	}
			//	else
			//		continue;
			//}
			...
			else if (!strcmp(pstrToken, "<Lambert>:"))
			{
				...
				continue;
			pMaterial->CreateShaderVariable(pd3dDevice, pd3dCommandList);
...
CGameObject *CGameObject::LoadFrameHierarchyFromFile(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature, CGameObject *pParent, FILE *pInFile, CShader *pShader, int *pnSkinnedMeshes)
{
	...
	for (; ; )
	{
		...
		if (!strcmp(pstrToken, "<Transform>:"))
		{
			...
			//pGameObject->m_xmf4x4ToParent._12 = mat._13;
			//pGameObject->m_xmf4x4ToParent._22 = mat._23;
			//pGameObject->m_xmf4x4ToParent._32 = mat._33;
			//pGameObject->m_xmf4x4ToParent._42 = mat._43;
			//pGameObject->m_xmf4x4ToParent._13 = mat._12;
			//pGameObject->m_xmf4x4ToParent._23 = mat._22;
			//pGameObject->m_xmf4x4ToParent._33 = mat._32;
			//pGameObject->m_xmf4x4ToParent._43 = mat._42;
		}
...
void CGameObject::CreateInstanceBuffer(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList,
	UINT nInstances, unordered_map<string, CB_OBJECT_INFO*>& instancedTransformBuffer)
{
	if (nInstances > 0)
	{
		...
		if (m_pSibling) m_pSibling->CreateInstanceBuffer(pd3dDevice, pd3dCommandList, nInstances, instancedTransformBuffer);
		if (m_pChild) m_pChild->CreateInstanceBuffer(pd3dDevice, pd3dCommandList, nInstances, instancedTransformBuffer);
	}
}