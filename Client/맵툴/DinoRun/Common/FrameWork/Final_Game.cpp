...
void Final_Game::ProcessInput()
{
	...
	else if (ImGui::IsAnyItemHovered()|| ImGui::IsAnyItemFocused())
		return;
...
void Final_Game::AnimateObjects()
{
	...
	if (m_pgameScene)
	{
		m_pgameScene->AnimateObjects(fTimeElapsed);
		m_pgameScene->Update(fTimeElapsed);
	}
}
...
void Final_Game::FrameAdvance()  //이곳에 update, rander 기능 모두 포함돼있음
{
	...
	if (g_pd3dSrvDescHeap) 
		m_pd3dCommandList->SetDescriptorHeaps(1, &g_pd3dSrvDescHeap);
	ImGui::Render();
	ImGui_ImplDX12_RenderDrawData(ImGui::GetDrawData(), m_pd3dCommandList.Get());

	Change_from_rt_to_present();   //후면버퍼를 랜더타겟에서 present로 변환
...
void Final_Game::Change_from_rt_to_present()
{
...
void Final_Game::Change_from_present_to_rt()
{
	D3D12_RESOURCE_BARRIER d3dResourceBarrier;
	::ZeroMemory(&d3dResourceBarrier, sizeof(D3D12_RESOURCE_BARRIER));
