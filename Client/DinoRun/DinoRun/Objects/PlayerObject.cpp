...
#define PLAYER_ANI_TIME_LENGTH 0.49f //플레이어의 애니메이션 길이
...
bool CPlayer::Update(float fTimeElapsed, CGameObject* target)
{
	...
	case ModelType::Item_Mound:
		...
		break;
	case ModelType::Item_FogBox:
		if (m_pCamera)
			m_pCamera->m_fFogStart = 5.0f;
		break;
	default:
		break;
	}
...
void CPlayer::FixedUpdate(float fTimeElapsed)
{
	...
	vel.y = 0;
	if (m_fWheelDegree != 0.0f && Vector3::Length(vel) > 0)   //머리의 조향각입력 & 시동걸린상태일때만 
	{
		float degree = m_fWheelDegree;  //현재 머리의 회전 각도

		//3.8f = 공룡의 머리 꼬리사이의 길이
		//57.3f = 각도를 degree값으로 변환하기위해 사용됨
		//--------------------
		float slipB = Vector3::DotProduct(m_xmf3Look, m_xmf3Velocity)
			/ (Vector3::Length(m_xmf3Look)*Vector3::Length(m_xmf3Velocity));  //슬립각도의 코사인값임.
	...
	{
		drag = 0.5f* 1.5f* AIR * 8.2f;   //1.5f= 마찰계수 AIR = 공기저항 , 8.2f = 정면 면적
		//드리프트시에는 저항을 받는 면적이 정면이 아닌 옆면이이때문에 면적이 더 큼.
	}
	else if (isShift)
	{
		drag = 0.5f* 1.0f* AIR * 4.0f;   //1.0f= 마찰계수 AIR = 공기저항 , 4.0f = 정면 면적
	}
	else
	{
	...
	float L = 3.8f, c = 1.9f, b = 1.9f; //L은 머리 꼬리사이 거리. c = 중심에서 머리까지 거리, b는 중심~꼬리 거리
	float h = 1.4f; //cg의 높이
...
CDinoRunPlayer::CDinoRunPlayer(string sModelName) : CPlayer()
{
	...
	SetPosition(XMFLOAT3(800.0f, 76.0f, 1150.0f)); 
...
CCamera *CDinoRunPlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	...
	case SPACESHIP_CAMERA:
		...
		m_pCamera->GenerateProjectionMatrix(1.01f, 1800.0f, EventHandler::GetInstance()->m_nWndClientWidth / EventHandler::GetInstance()->m_nWndClientHeight, 60.0f);
		m_pCamera->SetViewport(0, 0, EventHandler::GetInstance()->m_nWndClientWidth, EventHandler::GetInstance()->m_nWndClientHeight, 0.0f, 1.0f);
		m_pCamera->SetScissorRect(0, 0, EventHandler::GetInstance()->m_nWndClientWidth, EventHandler::GetInstance()->m_nWndClientHeight);
		break;
	case THIRD_PERSON_CAMERA:
		SetFriction(150.0f);
		SetGravity(XMFLOAT3(0.0f, -0.0f, 0.0f));
