...
VS_UI_OUTPUT VSUi(VS_UI_INPUT input, uint nInstanceID : SV_InstanceID)
{
	...
	output.num = gGameUiInfos[nInstanceID].uvX;
	output.TexC = float2(input.TexC.x + gGameUiInfos[nInstanceID].uvX, input.TexC.y + gGameUiInfos[nInstanceID].uvY);
	return(output);
}
...
VS_UI_OUTPUT VSUiNumber(VS_UI_INPUT input, uint nInstanceID : SV_InstanceID)
{
	...
	output.TexC.x = input.TexC.x + (uint(output.num) % 10) * 0.1;
	output.TexC.y = input.TexC.y + ((uint(output.num) / 10) * 0.5);
	return(output);
}
...
VS_UI_OUTPUT VSUiGuage(VS_UI_INPUT input, uint nInstanceID : SV_InstanceID)
{
	...
	if (output.position.x > 0)
		output.position.x = -output.position.x + output.position.x * 2 * gGameUiInfos[nInstanceID].uvX;
